...
//Rare exceptions to indicate a bug in the _passpartout code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_passpartout = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_passpartout]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.passpartout.module"
